...
current_params = dict(
    ...
    activation=activation  # keep the callable here for downstream code
    )

# Build a small, stable dict used for session-state comparison so UI actions
# (like downloading) don't accidentally clear frames due to callable identity changes.
serializable_params = dict(current_params)
serializable_params['activation'] = activation_options
if 'last_params' not in st.session_state:
    st.session_state['last_params'] = serializable_params
elif st.session_state['last_params'] != serializable_params:
