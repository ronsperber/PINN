...
st.write(
    "To see a differential equation solved using a PINN, select an equation type on the left, adjust any desired parameters, and press solve."
)
...
if meta.get("is_system", False):
    ...
    if meta.get("x0_positive"):
        ...
        x_start = st.sidebar.number_input(
            "x start (must be > 0)",
            min_value=x_start_min,
            value=x0,
            key="x_start_positive",
        )
        x_end = st.sidebar.number_input(
            "x end",
            min_value=max(x_start - EPS, 1e-6),
            value=x_start + 5.0,
            key="x_end_pos",
        )
    else:
        x_start = st.sidebar.number_input(
            "x start", value=x0, min_value=x0 - 5.0 - EPS, key="x_start_default"
        )
        x_end = st.sidebar.number_input(
            "x end", value=x_start + 2.0, min_value=x_start - EPS, key="x_end_default"
        )
...
with st.sidebar.expander("Neural Network Parameters", expanded=False):
    st.caption(
        "Tweak only if the solver struggles or you want to experiment. Choosing very large values for epochs, number of hidden layers, and/or layer width could make the solver slow."
    )
    n_points = st.number_input(
        "Number of points in interval", min_value=10, value=100, step=10
    )
    epochs = st.number_input(
        "Epochs", value=500, step=100, min_value=10, max_value=10000
    )
    lr = st.number_input("Learning rate", value=1e-3, format="%.5f")
    num_hidden_layers = st.number_input("Hidden layers", value=2, step=1)
    layer_width = st.number_input(
        "Layer width", value=64, step=1, min_value=2, max_value=1024
    )
    activation_option = st.selectbox(
        "Activation function", ["Tanh", "Sine", "Swish", "Softplus"], index=0
    )
...
if solve_clicked:
    ...
    if meta.get("x0_positive", False) and x0 <= 0:
        ...
        st.sidebar.error(
            "This ODE requires the interval start to be > 0. Please set x start to a positive value."
        )
    else:
        ...
        if callable(true_factory):
            ...
            if true_sol is None:
                ...
                st.sidebar.warning(
                    "Analytic true solution unavailable for selected parameters."
                )
        ...
        def _progress_callback(epoch, train_loss, val_loss):
            ...
            progress_text.text(
                f"Epoch {epoch}/{epochs} — train_loss: {train_loss:.6e}, val_loss: {val_loss:.6e} — ETA: {eta_str}"
            )

        with st.spinner("Solving..."):
            checkpoint_every = min(max(1, epochs // 25), 20)
            y_trial, checkpoints = ode_solve.ode_solve(
                F=F,
                a=x0,
                ics=ics,
                NN=NN,
        ...
        for checkpoint in checkpoints + [("final", y_trial)]:
            ...
            if isinstance(checkpoint[0], int):
                ...
                if meta.get("order", 1) == 1:
                    ...
                    title = (
                        f"Final Solution to {ode}, y({x0}) = {y0}, y'({x0}) = {yprime0}"
                    )
                epoch_val = "final"
            frames.append(
                {
...
if "frames" in st.session_state:
    ...
    if meta.get("is_system", False):
        ...
        fig.update_layout(
            xaxis_title="y₁(t)", yaxis_title="y₂(t)", title="Phase-space trajectory"
        )
    else:
        fig.add_trace(
            go.Scatter(
                x=x,
                y=frames[final_idx]["y_pred"].flatten(),
                mode="lines",
                name="Prediction",
            )
        )
        if has_true:
            fig.add_trace(
                go.Scatter(
                    x=x,
                    y=frames[final_idx]["y_true"].flatten(),
                    mode="lines",
                    name="True Solution",
                    line=dict(dash="dash"),
                )
    ...
    for i, fr in enumerate(frames):
        if meta.get("is_system", False):
            data = [
                go.Scatter(
                    x=fr["y_pred"][:, 0].flatten(),
                    y=fr["y_pred"][:, 1].flatten(),
                    mode="lines",
                    name="Prediction",
                )
            ]
            ...
            data = [
                go.Scatter(
                    x=x, y=fr["y_pred"].flatten(), mode="lines", name="Prediction"
                )
            ]
        ...
        if fr["y_true"] is not None:
            if meta.get("is_system", False):
                data.append(
                    go.Scatter(
                        x=fr["y_true"][:, 0].flatten(),
                        y=fr["y_true"][:, 1].flatten(),
                        mode="lines",
                        name="True Solution",
                        line=dict(dash="dash"),
                    )
        ...
        plotly_frames.append(
            go.Frame(data=data, name=str(i), layout=dict(title=frame_title))
        )
    n_frames = len(frames)
    ...
    fig.update_layout(
        updatemenus=updatemenus, sliders=sliders, title=frames[final_idx]["title"]
    )
...
def frames_to_gif(plotly_frames, x, fps=10, is_system=False, max_line_length=40):
    ...
    for fr in plotly_frames:
        ...
        fig.update_layout(
            ...
            ),  # t=120 gives more space for wrapped title
        )
...
if (
    make_gif
    and "plotly_frames" in st.session_state
    and "gif_bytes" not in st.session_state
):
...
if "frames" in st.session_state:
    ...
    with st.expander("Show ODE residuals", expanded=False):
        ...
        res_fig.update_layout(
            title="ODE residual (per-frame)",
            xaxis_title="Checkpoint",
            yaxis_title="Mean ODE residual",
